...
SGNsGrab.NsGrab.prototype = {
	...
	isUpload : false,
	autoLogin : SGNsUtils.getBoolPref('extensions.screengrab.autoLogin',false),
	
    /** entire page */
	setUpload :function(isUpload) {
		this.isUpload = isUpload;
		this.autoLogin = SGNsUtils.getBoolPref('extensions.screengrab.autoLogin',false);
	},
    ...
    saveToFile : function(dataUrl, format) {    
		if (this.isUpload)
		{
			var nsFile = Screengrab.defaultFileName() + "." + format;
			this.nSFile = nsFile;        					
	        var binaryInputStream = SGNsUtils.dataUrlToBinaryInputStream(dataUrl);
			if (this.autoLogin)
				Upload.login();
			Upload.uploadfile(binaryInputStream,nsFile);			
		}
		else
		{
